...
Then the error response has a status code of 202 and matches
	| code | 202      |
	| text | ACCEPTED |
...
Then the error response has a status code of 201 and matches
	| code | 201     |
	| text | CREATED |
...
Then the error response has a status code of 204 and no body
...
Then the error response has a status code of 200 and matches
	| code | 200 |
	| text | OK  |
