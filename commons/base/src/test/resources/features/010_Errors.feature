...
Then the error response has a status code of 403 and matches
	| code | 403       |
	| text | FORBIDDEN |
...
Then the error response has a status code of 405 and matches
	| code | 405                |
...
Then the error response has a status code of 404 and matches
	| code | 404       |
	| text | NOT_FOUND |
...
Then the error response has a status code of 408 and matches
	| code | 408             |
...
Then the error response has a status code of 401 and matches
	| code | 401          |
	| text | UNAUTHORIZED |
...
Then the error response has a status code of 208 and matches
	| code | 208              |
...
And prepare data table "validationInfo"
	| field  | value  | error  |
	| string | null   | string |
And the error response contains "validationInfo" objects in "validations"
	| field | value  | error            |
